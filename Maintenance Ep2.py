...
GUI.title('โปรแกรมซ่อมบำรุง v.0.0.1 by Engi3')
...
def Save():
    ...
    print(machine, problem, reporter, date)
    ...
    with open('log.txt','r',encoding='utf-8') as file:
        ...
        for t in text:
            data = t.strip().split(',')
            table.tag_configure('large_font', font=(None, 15))
            table.insert('','end',values=data, tags='large_font')

B = Button(F1, text='บันทึก',font=(None, 20), command=Save).pack(ipadx=50, ipady=30, pady=20)
...
table = ttk.Treeview(F2, column=header,show='headings',height=20)
table.pack()
